...
runCommand="docker run --rm --name maven --network host -i -v "$PWD"/src:/usr/src/app/src -v "$PWD"/reports:/usr/src/app/target/site/serenity deosite/maven verify"
...
start_browser() {
  docker run -d --network host --name selenium --shm-size 2g selenium/standalone-"$browser":4.0.0-beta-1-prerelease-20210114
}
